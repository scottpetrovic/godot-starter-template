...
@export var item_name: String = "ITEM": set = _set_item_name
@export var item_font_color: Color = Color.BLACK: set = _set_item_font_color

func _ready():
	$Button.connect("mouse_entered", Callable(self, "_add_text_highlight"))
	$Button.connect("mouse_exited", Callable(self, "_remove_text_highlight"))
...
func _set_item_font_color(value: Color):
	...
	$Button.add_theme_color_override("font_color_hover", value)
	$SelectedBefore.add_theme_color_override("font_color", value)
	$SelectedAfter.add_theme_color_override("font_color", value)
